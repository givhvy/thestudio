...
void BottomDock::paint(juce::Graphics& g)
{
    ...
    const char* trackNames[] = { "KICK", "SNARE", "HIHAT", "CLAP", "PERC", "BASS", "LEAD", "PAD" };
    const float trackVolumes[] = { 0.78f, 0.64f, 0.55f, 0.46f, 0.50f, 0.70f, 0.62f, 0.58f };
    int numTracks = 8;
    ...
    for (int i = 0; i < numTracks; ++i)
    {
        ...
        // Fader fill (blue)
        float fillH = faderHeight * trackVolumes[i];
        auto fill = juce::Rectangle<float>((float)cx - 2, (float)faderBot - fillH, 4, fillH);
        juce::ColourGradient bGrad(Theme::blue.brighter(0.3f), 0.0f, fill.getY(),
                                     Theme::blue.darker(0.3f), 0.0f, fill.getBottom(), false);
        ...
        g.drawText(trackNames[i], sx, mxp.getBottom() - 18, stripW, 14, juce::Justification::centred);
    }
