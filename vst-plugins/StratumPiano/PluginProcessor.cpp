...
StratumPianoAudioProcessor::StratumPianoAudioProcessor()
...
juce::AudioProcessorValueTreeState::ParameterLayout StratumPianoAudioProcessor::createParameterLayout()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterFloat>("decay", "Decay",
        juce::NormalisableRange<float>(0.2f, 5.0f, 0.001f), 2.2f));
    return { params.begin(), params.end() };
}

void StratumPianoAudioProcessor::prepareToPlay(double sampleRate, int)
{
...
void StratumPianoAudioProcessor::releaseResources()
{
}

bool StratumPianoAudioProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
    const auto& out = layouts.getMainOutputChannelSet();
    return out == juce::AudioChannelSet::mono() || out == juce::AudioChannelSet::stereo();
}

double StratumPianoAudioProcessor::midiToHz(int midiNote)
{
    return 440.0 * std::pow(2.0, ((double)midiNote - 69.0) / 12.0);
}

int StratumPianoAudioProcessor::noteNameToMidi(const juce::String& name)
{
...
juce::File StratumPianoAudioProcessor::findSampleFolder() const
{
    juce::Array<juce::File> candidates;
    const auto moduleFile = juce::File::getSpecialLocation(juce::File::currentApplicationFile);
    candidates.add(moduleFile.getParentDirectory().getChildFile("Samples").getChildFile("Piano"));
    candidates.add(moduleFile.getParentDirectory().getParentDirectory().getChildFile("Samples").getChildFile("Piano"));
    candidates.add(moduleFile.getParentDirectory().getParentDirectory().getParentDirectory().getChildFile("Samples").getChildFile("Piano"));
    candidates.add(juce::File("F:/PlaygroundTest/flstudioclonee/vst-plugins/Samples/Piano"));
...
void StratumPianoAudioProcessor::loadSamples()
{
    samples_.clear();
...
int StratumPianoAudioProcessor::findClosestSampleIndex(int midiNote) const
{
    if (samples_.empty())
        return -1;
...
void StratumPianoAudioProcessor::startNote(int midiNote, float velocity)
{
...
void StratumPianoAudioProcessor::stopNote(int midiNote)
{
    for (auto& voice : voices_)
    {
...
void StratumPianoAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int sample = 0; sample < buffer.getNumSamples(); ++sample)
    {
        ...
        for (auto& voice : voices_)
        {
            ...
            if (voice.releasing)
            {
                const float rel = std::exp((float)(-voice.releaseAge / 0.22));
                voiceOut *= rel;
                voice.releaseAge += 1.0 / sampleRate_;
                if (std::abs(voiceOut) < 0.0002f && voice.releaseAge > 0.35)
                {
                    voice.active = false;
                    continue;
                }
...
float StratumPianoAudioProcessor::renderSampleVoice(Voice& voice)
{
...
float StratumPianoAudioProcessor::renderSynthVoice(Voice& voice, float tone, float decay)
{
    const double twoPi = juce::MathConstants<double>::twoPi;
    const double dt = 1.0 / sampleRate_;
    const float attack = voice.age < 0.006 ? (float)(voice.age / 0.006) : 1.0f;
    const float bodyEnv = std::exp((float)(-voice.age / juce::jmax(0.05f, decay)));
    ...
    const float bright = 0.08f + tone * 0.22f;
    const float value = std::sin((float)voice.phase) * 0.86f
                      + std::sin((float)voice.phase2) * bright
                      + std::sin((float)voice.phase3) * bright * 0.45f;
    voice.age += dt;
    return value * env;
}

juce::AudioProcessorEditor* StratumPianoAudioProcessor::createEditor()
{
    return new StratumPianoAudioProcessorEditor(*this);
}

void StratumPianoAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
    if (auto xml = parameters.copyState().createXml())
        copyXmlToBinary(*xml, destData);
}

void StratumPianoAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
    return new StratumPianoAudioProcessor();
}
